...
use tempo_cli::test_utils::{TestContext, with_test_db_async};
use anyhow::Result;
...
async fn test_project_crud_operations() {
    with_test_db_async(|ctx| async move {
        ...
        let updated = ProjectQueries::update_name(&ctx.connection(), project_id, "Updated Project".to_string())?;
        assert!(updated);
...
async fn test_session_date_range_filtering() {
    with_test_db_async(|ctx| async move {
        ...
        let today_sessions = SessionQueries::list_by_date_range(&ctx.connection(), today_start, today_end)?;
        ...
        let yesterday_start = yesterday.date_naive().and_hms_opt(0, 0, 0).unwrap().and_utc();
        let yesterday_end = yesterday.date_naive().and_hms_opt(23, 59, 59).unwrap().and_utc();
        let yesterday_sessions = SessionQueries::list_by_date_range(&ctx.connection(), yesterday_start, yesterday_end)?;
        ...
        let wide_sessions = SessionQueries::list_by_date_range(&ctx.connection(), yesterday - Duration::hours(1), tomorrow)?;
...
async fn test_multiple_projects_and_sessions() {
    with_test_db_async(|ctx| async move {
        ...
        assert_eq!(found2.description, Some("Tempo tracked project".to_string()));
...
async fn test_edge_cases_and_error_conditions() {
    with_test_db_async(|ctx| async move {
        ...
        let not_updated = ProjectQueries::update_name(&ctx.connection(), 99999, "New Name".to_string())?;
        assert!(!not_updated);
        ...
        let now = Utc::now();
        let empty_range = SessionQueries::list_by_date_range(&ctx.connection(), now, now + Duration::hours(1))?;
